...
import type { PlayerState, Peer, ServerMessage, ClientMessage, ChatMessage } from '../../shared/types/realtime'
...
export default defineWebSocketHandler({
    ...
    switch (msg.t) {
      case 'join': {
        ...
        if (prevRoom && prevRoom !== placeId) {
          ...
          peer.publish(prevRoom, JSON.stringify({ t: 'leave', id: identity.id } satisfies ServerMessage))
        }
        ...
        peer.publish(placeId, JSON.stringify({ t: 'join', peer: playerState } satisfies ServerMessage))
        break
      }
      ...
      case 'move': {
        ...
        const x = clamp(msg.x, -100, 100)
        const y = clamp(msg.y ?? 0, -50, 50)
        const z = clamp(msg.z, -100, 100)
        ...
        peer.publish(placeId, JSON.stringify({
          t: 'move', id: identity.id, x, y, z, rotation,
  ...
  close(peer) {
    ...
    if (placeId) {
      ...
      peer.publish(placeId, JSON.stringify({ t: 'leave', id: identity.id } satisfies ServerMessage))
    }
